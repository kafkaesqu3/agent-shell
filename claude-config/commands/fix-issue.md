...
error messages, or domain-specific concepts. If so, search for:
...
unfamiliar API, or an approach that isn't working -- use web
search to find solutions rather than spinning.
...
check prose accuracy, confirm rendering). For code changes, review
the diff against the upstream main branch. Produce a list of
